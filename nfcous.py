...
def parse_page(page_content, page_number):
    ...
    if wrap_c_div:
        ...
        # 查找所有 <li> 标签
        li_tags = wrap_c_div.find_all("li")
        
        # 遍历每个 <li> 标签，提取文件名和下载链接
        for li_tag in li_tags:
            # 提取 <p class="p1"> 标签中的期号信息
            p_tag = li_tag.find("p", class_="p1")
            if p_tag:
                # 使用 UTF-8 解码确保文件名不会乱码
                file_name = p_tag.get_text(strip=True).encode('utf-8').decode('utf-8')
                print(f"提取的文件名: {file_name}")
            else:
                print("未找到期号信息，使用默认文件名。")
                file_name = f"安全期刊_{page_number}"
            
            # 查找 <a> 标签中包含 PDF 下载链接的 href
            download_link = li_tag.find("a", class_="a1", href=True)
            if download_link:
